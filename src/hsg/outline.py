...
SCHEMA_SMALL = """{
  ...
  "outro": "结尾词（50-80 字，收束本期主题、留有余味，可直接朗读口播，对称开篇钩子；不要喊口号）",
...
【层次要求｜2026-09-17 用户指定：由浅及深、有层次、语义连贯】
· 整体是一条**由浅及深的梯子**，每一章只比上一章深一层，不许跳级：
    第 1 章：只讲"发生了什么"——具体的人、动作、场景，先给事实与悬念，**不下结论**
    中间章节：先讲"他靠什么做到" → 再讲"制度上为什么能 / 为什么本来不行"
              → 再讲"代价与风险落在谁身上"
    末章：最后才给判断与余韵（这件事说明了什么），不要喊口号
· 章与章之间要**接得上**：每一章的开头要接住上一章留下的那个问题，
  每一章的结尾要自然留下下一章要回答的问题（问题链，而不是并列的几条知识点）。
· 同一概念前后用**同一个词**（别一会儿"辅政大臣"、一会儿"托孤重臣"）；
  每章要点里写清"本章回答上一章的哪个问题"。"""
