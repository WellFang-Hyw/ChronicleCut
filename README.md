...
```
run.bat agent                     看现在卡在哪、下一步该敲什么（唯一的人机交接点）
run.bat agent --stage 1 -t "题材"  脚本 + 分镜 + **素材需求清单**，出完停下
（人工）python scripts\import_clip.py --file <片>.mp4 --id sg_01 --title 三国演义 \
        --year 1994 --people 曹操 --era 东汉末 --slots s04_sh1,s04_sh2
run.bat agent --stage 2            AI 排镜头 → 渲染 → 自检 → 出片
```
