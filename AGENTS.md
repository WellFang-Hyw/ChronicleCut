...
run.bat test                              :: 零成本回归测试（456 项，不调 API）
...
29. **取景单（sources.py）里的候选片源一律标「未核实」，绝不自动绑槽位**。
   ...
   主演也是拼的），所以：① 提示词禁止它给集数/时间码，只允许**剧情阶段**描述
   （`FABRICATED` 正则兜底，命中就替换成占位句）；② 每部剧必须给**主演或播出平台**锚点，
   给不出的渲染成「很可能是编的」；③ 再跑一次**剧名自检**（`_selfcheck`）让它自己交叉核对，
